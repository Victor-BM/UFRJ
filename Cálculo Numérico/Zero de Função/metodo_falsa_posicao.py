...
x = sp.symbols('x')
f_simbolica = -x**4 - sp.log(x, 10) #definir função
f_numerica = sp.lambdify(x, f_simbolica, 'numpy')

def falsa_posicao(xo, x1, erro, arredondamento):
    xo = xo
    x1 = x1
    erro = erro
    i = 0
    ...
    while i < 100:
        ...
        if f_numerica(xo)*f_numerica(x_novo) < 0:
            ...
            if abs(x_novo - x1) <= erro:
                print(f'raiz = {x_novo}\n{i} iterações')
                return
    print(f'Método da Falsa Posição após 100 iterações: raiz = {x_novo}')
...
def main():
    ...
    falsa_posicao(xo, x1, erro, arredondamento)
main()